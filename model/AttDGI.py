    ...
    def weights_init(self, m):
        if isinstance(m, nn.Linear):
            ...
            if m.bias is not None:
                m.bias.data.fill_(0.0)
    #
    def forward(self, drug_hidden_out, protein_hidden_out, real_drug, real_protein, fake_drug, fake_protein, real_protein_id, real_drug_id, fake_protein_id, fake_drug_id, msk=None, samp_bias1=None,
                samp_bias2=None):

        S_u_One = self.read(drug_hidden_out, msk).unsqueeze(0)  # 1*hidden_dim
        S_i_One = self.read(protein_hidden_out, msk).unsqueeze(0)  # 1*hidden_dim
        S_Two = self.lin(torch.cat((S_u_One, S_i_One), dim = -1)) # 1 * hidden_dim
        S_Two = self.sigm(S_Two)  # hidden_dim  need modify

        real_sub_u_Two = self.att(real_protein_id, real_drug, 0) + real_drug  # hidden_dim
        real_sub_i_Two = self.att(real_drug_id, real_protein, 1) + real_protein  # hidden_dim
        fake_sub_u_Two = self.att(fake_protein_id, fake_drug, 0) + fake_drug  # hidden_dim
        fake_sub_i_Two = self.att(fake_drug_id, fake_protein, 1) + fake_protein  # hidden_dim
